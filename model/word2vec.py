...
def naiveSoftmaxLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset):
    ...
    gradCenterVec = np.transpose(outsideVectors).dot(y_hat - y)
    gradOutsideVecs = np.array(
        [((y_hat[w] - y[w]) * centerWordVec) for w in range(outsideVectors.shape[0])]
    )
...
def skipgram(
    ...
    for word in outsideWords:
        ...
        loss += wordLoss
        gradCenterVecs[currentCenterWordIdx] += wordGradCenterVec
